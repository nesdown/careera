...
function writeWaitlist(entries) {
  try {
    writeFileSync(WAITLIST_FILE, JSON.stringify(entries, null, 2), 'utf8');
  } catch (err) {
    console.error('[waitlist] Failed to write file:', err.message);
  }
}
...
app.post('/api/waitlist', async (req, res) => {
  try {
    ...
    const entry = { email, signedUpAt: new Date().toISOString(), ip: req.ip };
    entries.push(entry);
...
app.get('/api/waitlist', (req, res) => {
  // Protected with a simple secret so the list can be viewed without a dashboard
